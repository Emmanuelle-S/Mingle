...
const MyTextInput = ({ label, ...props }) => {
  // = fonction fléchée qui prend comme argument un objet destructuré : contient au moins une propriété label, ainsi que toutes les autres propriétés (props) qui peuvent être passées à l'élément <input>.
...
const MyTextarea = ({ label, ...props }) => {
  // fonction fléchée qui prend comme argument un objet destructuré, qui contient au moins une propriété label, ainsi que toutes les autres propriétés (props) qui peuvent être passées à l'élément <textarea>.
  const [field, meta] = useField(props);
...
const MyFileInput = ({ label, ...props }) => {
  // fonction fléchée qui prend comme argument un objet destructuré. Ce objet contient au moins une propriété label, ainsi que toutes les autres propriétés (props) qui peuvent être passées à l'élément <input>.
...
const EditProfil = () => {
  return (
    <Formik
      ...
      onSubmit={(values) => {
        console.log(values);
        // onSubmit est appelée lorsque le formulaire est soumis. pour l'instant elle logue les valeurs du formulaire dans la console.
      }}
    >
      <Form className="max-w-screen-lg mx-auto px-6 py-8 rounded-lg shadow-lg">
        ...
        <div className="mt-4 text-center">
          <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white py-4 px-12 rounded-lg text-lg font-mediumfocus:outline-none focus:shadow-outline">
            Enregistrer
          </button>
        </div>
      </Form>
